...
async def _fetch_api(
    ...
    if token:
        headers["Authorization"] = f"Token {token.strip()}" # Ensure no leading/trailing whitespace in token
    ...
    try:
        async with session.request(
            ...
            try:
                ...
                # or {"detail": "Not found."}
                data = await resp.json(content_type=None) # Allow any content type if server misreports
            ...
            if isinstance(data, dict):
                ...
                elif "id" in data and data["id"] == "error" and "msg" in data: # Old error format
        ...
        raise VNDBAPIError(f"Request to {url} timed out after {CLIENT_TIMEOUT_SECONDS} seconds.", status_code=None)
        ...
        raise VNDBAPIError(f"AIOHTTP client error during request to {url}: {e}", status_code=None)
