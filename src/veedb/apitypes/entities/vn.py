...
class VNImageInfo(ImageCommon): # For vn.image and vn.screenshots
    ...
    release: Optional[Dict[str, Any]] = None # e.g. {'id': 'r123', 'title': 'Release Title'}
...
class VNVoiceActor: # vn.va
    ...
    staff: Dict[str, Any] = field(default_factory=dict) # e.g. {'id': 's1', 'name': 'Staff Name'}
    character: Dict[str, Any] = field(default_factory=dict) # e.g. {'id': 'c1', 'name': 'Char Name'}
...
class VN:
    ...
    alttitle: Optional[str] = None # Alternative title, typically original script if olang differs
    ...
    devstatus: Optional[DevStatusEnum] = None # 0: Finished, 1: In development, 2: Cancelled
    released: Optional[ReleaseDate] = None # Date of first known release

    languages: List[LanguageEnum] = field(default_factory=list) # Languages VN is available in
    platforms: List[PlatformEnum] = field(default_factory=list) # Platforms VN is available on

    image: Optional[VNImageInfo] = None # Main cover image info

    length: Optional[int] = None # Rough length estimate: 1 (Very short) to 5 (Very long)
    length_minutes: Optional[int] = None # Average of user-submitted play times in minutes
    ...
    va: List[VNVoiceActor] = field(default_factory=list) # Voice actors linked to characters in this VN
